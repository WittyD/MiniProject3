			...
			for phn_file in os.listdir():
				if phn_file[-1]=='n':
					...
					for i in range(len(data['3'])):
						if data.iloc[i]['3'] not in lis:
							file_name=phn_file[:len(phn_file)-4]
							try:
								Audio = lb.load(file_name+'.wav',sr=sample_rate)
								Audio1=Audio[0]
								Audio2=Audio1[data.iloc[i]['1']:data.iloc[i]['2']]
								# mfcc = lb.feature.mfcc(y=np.array(Audio2), sr=sample_rate,n_mfcc=13,hop_length=len(Audio2)*2)
								fft = lb.stft(y = np.array(Audio2),n_fft = 512,hop_length=240)
								abs = np.abs(np.mean(fft))
								mfcc = lb.feature.mfcc(y=np.array([abs]), sr=16000,n_mfcc=13,hop_length = 240, n_fft = 512)
...
print(final_data)
final_data.to_csv('witty_false.csv')
